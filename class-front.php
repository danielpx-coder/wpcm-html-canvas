...
root@centralmidia1:/var/www/tribunapopular.com.br/htdocs/wp-content/plugins/wpcm-html-canvas/includes# <?php
/**                                                                                                    cat class-admin.php
                                                                                                       ls
class-admin.php  class-css-scoper.php  class-front.php  class-sanitizer.php
root@centralmidia1:/var/www/tribunapopular.com.br/htdocs/wp-content/plugins/wpcm-html-canvas/includes# cat class-css-scoper.php
<?php
/**
 * Motor CSS tokenizado com escopo.
 *
 * Usa tokenização por chaves em vez de regex puro, lidando corretamente
 * com @media, @supports, @layer, @container, @keyframes, @font-face, @import.
 *
 * @package WPCM\HTMLCanvas
 */
...
class CSS_Scoper {
    ...
    /** @var string[] At-rules que não devem ter seletores escopados */
    private const PRESERVE_ATRULES = [
        'font-face', 'keyframes', '-webkit-keyframes', '-moz-keyframes',
    ];

    /** @var string[] At-rules que contêm seletores internos (processar recursivamente) */
    private const NESTED_ATRULES = [
        'media', 'supports', 'layer', 'container', 'scope',
        '-webkit-media', '-moz-media',
    ];

    public function __construct( string $wrapper_selector ) {
        $this->wrapper = $wrapper_selector;
    }

    /**
     * Processa um bloco CSS completo (conteúdo de um <style>).
     */
    public function scope( string $css ): string {
        $css = trim( $css );
        if ( '' === $css ) return '';

        $tokens = $this->tokenize( $css );
        return $this->process_tokens( $tokens );
    }

    /**
     * Tokeniza CSS em uma árvore de blocos.
     *
     * Cada token é:
     * - ['type' => 'rule',   'selector' => '...', 'declarations' => '...']
     * - ['type' => 'atrule', 'name' => 'media', 'prelude' => '(max-width:768px)', 'children' => [...]]
     * - ['type' => 'import', 'raw' => '@import url(...)']
     *
     * @return array
     */
    private function tokenize( string $css ): array {
        ...
        while ( $i < $len ) {
            ...
            // @import (não tem bloco {})
            if ( substr( $css, $i, 7 ) === '@import' ) {
                $semi = strpos( $css, ';', $i );
                if ( $semi !== false ) {
                    $tokens[] = [
                        'type' => 'import',
                        'raw'  => substr( $css, $i, $semi - $i + 1 ),
                    ];
                    $i = $semi + 1;
                } else {
                    $i = $len;
            ...
            // @charset (não tem bloco {})
            if ( substr( $css, $i, 8 ) === '@charset' ) {
                $semi = strpos( $css, ';', $i );
                $i = ( $semi !== false ) ? $semi + 1 : $len;
                continue;
            }

            // At-rules com bloco
            if ( $css[ $i ] === '@' ) {
                // Extrai nome da at-rule
                $rest = substr( $css, $i + 1 );
                if ( preg_match( '/^([\w-]+)\s*/A', $rest, $m ) ) {
                    $atrule_name = strtolower( $m[1] );
                    $prelude_start = $i + 1 + strlen( $m[0] );

                    // Encontra o { de abertura
                    $brace_open = strpos( $css, '{', $prelude_start );
                    if ( $brace_open === false ) {
                        $i = $len;
                        continue;
                    }

                    $prelude = trim( substr( $css, $prelude_start, $brace_open - $prelude_start ) );

                    // Encontra o } de fechamento (respeitando aninhamento)
                    $inner_start = $brace_open + 1;
                    $inner_end   = $this->find_closing_brace( $css, $brace_open );
                    ...
                    $tokens[] = [
                        ...
                        'name'     => $atrule_name,
                        'prelude'  => $prelude,
                        'children' => $this->tokenize( $inner_css ),
                        'raw'      => substr( $css, $i, $inner_end + 1 - $i ),
                    ];
            ...
            // Regra normal: selector { declarations }
            $brace = strpos( $css, '{', $i );
            if ( $brace === false ) break;

            $selector = trim( substr( $css, $i, $brace - $i ) );

            $close = $this->find_closing_brace( $css, $brace );
            if ( $close === false ) break;

            $declarations = substr( $css, $brace + 1, $close - $brace - 1 );
    ...
    /**
     * Encontra a } que fecha um bloco aberto em $pos.
     */
    private function find_closing_brace( string $css, int $open_pos ): ?int {
        $depth = 1;
        $len   = strlen( $css );
        $i     = $open_pos + 1;

        while ( $i < $len && $depth > 0 ) {
            // Pula strings e comentários
            $ch = $css[ $i ];
            if ( $ch === '"' || $ch === "'" ) {
                $i = $this->skip_string( $css, $i );
                continue;
            }
    ...
    /**
     * Pula uma string CSS (delimitada por " ou ').
     */
    private function skip_string( string $css, int $pos ): int {
        $quote = $css[ $pos ];
        $len   = strlen( $css );
        $i     = $pos + 1;
        while ( $i < $len ) {
    ...
    /**
     * Processa tokens, aplicando escopo.
     */
    private function process_tokens( array $tokens ): string {
        $out = '';
        ...
        foreach ( $tokens as $token ) {
            switch ( $token['type'] ) {
                ...
                case 'atrule':
                    $out .= $this->process_atrule( $token );
                    break;

                case 'rule':
                    $out .= $this->scope_rule( $token['selector'], $token['declarations'] );
                    break;
            }
        }
    ...
    /**
     * Processa uma at-rule.
     */
    private function process_atrule( array $token ): string {
        $name = $token['name'];

        // At-rules preservadas integralmente (sem escopo nos seletores internos)
        foreach ( self::PRESERVE_ATRULES as $p ) {
            if ( $name === $p || str_ends_with( $name, $p ) ) {
                return $token['raw'] . "\n";
            }
        }

        // At-rules com seletores internos (processar recursivamente)
        $is_nested = false;
        foreach ( self::NESTED_ATRULES as $n ) {
            if ( $name === $n || str_ends_with( $name, $n ) ) {
                $is_nested = true;
                break;
            }
        }
        ...
        if ( $is_nested ) {
            $inner = $this->process_tokens( $token['children'] );
            return "@{$name} {$token['prelude']} {\n{$inner}}\n";
        }

        // Qualquer outra at-rule: preserva como está
        return $token['raw'] . "\n";
    }

    /**
     * Escopa os seletores de uma regra CSS.
     */
    private function scope_rule( string $selector_raw, string $declarations ): string {
        $w = $this->wrapper;

        $selectors = $this->split_selectors( $selector_raw );
        $scoped    = [];
        ...
        foreach ( $selectors as $sel ) {
            ...
            // html .x, body .x → wrapper .x
            elseif ( preg_match( '/^(?:html|body)\s+(.+)$/i', $sel, $m ) ) {
                $scoped[] = $w . ' ' . $m[1];
            }
            // body.class → wrapper.class
            elseif ( preg_match( '/^(?:html|body)(\.[^\s].*)$/i', $sel, $m ) ) {
                $scoped[] = $w . $m[1];
            }
            // body>.x, body>div → wrapper>.x
            elseif ( preg_match( '/^(?:html|body)\s*>\s*(.+)$/i', $sel, $m ) ) {
                $scoped[] = $w . ' > ' . $m[1];
            }
            // body~.x, body+.x → wrapper .x
            elseif ( preg_match( '/^(?:html|body)\s*[~+]\s*(.+)$/i', $sel, $m ) ) {
                $scoped[] = $w . ' ' . $m[1];
            }
            // * → wrapper *
            ...
            // Qualquer outro → wrapper selector
            else {
                $scoped[] = $w . ' ' . $sel;
            }
        }
    ...
    /**
     * Divide seletores por vírgula, respeitando parênteses.
     * Lida com :is(), :where(), :not(), :has() que contêm vírgulas internas.
     */
    private function split_selectors( string $raw ): array {
        $selectors = [];
        $current   = '';
        $depth     = 0;
        $len       = strlen( $raw );
...
root@centralmidia1:/var/www/tribunapopular.com.br/htdocs/wp-content/plugins/wpcm-html-canvas/includes# <?php
/**                                                                                                    cat class-css-scoper.php
                                                                                                       ls
class-admin.php  class-css-scoper.php  class-front.php  class-sanitizer.php
root@centralmidia1:/var/www/tribunapopular.com.br/htdocs/wp-content/plugins/wpcm-html-canvas/includes# cat class-front.php
<?php
/**
 * Frontend: shortcode, template override e proteção SEO/social.
 *
 * @package WPCM\HTMLCanvas
 */
...
class Front {
    ...
    private function __construct() {
        add_shortcode( 'wpcm_canvas', [ $this, 'shortcode' ] );

        add_filter( 'no_texturize_shortcodes', function( array $s ): array {
            $s[] = 'wpcm_canvas';
            return $s;
        });

        add_filter( 'template_include', [ $this, 'maybe_override_template' ], 99999 );

        $this->register_social_filters();
    }
    ...
    public function shortcode( $atts ): string {
        ...
        foreach ( $snippets as $s ) {
            ...
            $wid  = 'wpcm-block-' . esc_attr( $s['id'] );
            $code = do_shortcode( $s['code'] ?? '' );

            return '<div id="' . $wid . '" class="wpcm-canvas-block">' . $code . '</div>';
        }

        return '';
    }

    public function maybe_override_template( string $tpl ): string {
        if ( ! is_singular() || is_preview() ) return $tpl;

        $pid = get_the_ID();
        if ( ! $pid ) return $tpl;
        if ( 'isolated' !== get_post_meta( $pid, '_wpcm_mode', true ) ) return $tpl;

        $snippets = get_post_meta( $pid, '_wpcm_snippets', true );
        if ( empty( $snippets ) || ! is_array( $snippets ) ) return $tpl;

        $canvas = WPCM_PATH . 'templates/canvas-fullpage.php';
        return file_exists( $canvas ) ? $canvas : $tpl;
    }

    private function register_social_filters(): void {
        $cb = [ $this, 'strip_wpcm' ];

        foreach ( [ 'get_the_excerpt', 'the_excerpt', 'the_excerpt_rss', 'wp_trim_excerpt' ] as $h ) {
            add_filter( $h, $cb, 1 );
        }

        foreach ( [ 'wpseo_opengraph_desc', 'wpseo_metadesc', 'wpseo_twitter_description' ] as $h ) {
            add_filter( $h, $cb, 1 );
        }
        ...
        foreach ( [
            ...
            'rank_math/opengraph/twitter/og_description',
        ] as $h ) {
            add_filter( $h, $cb, 1 );
        }

        foreach ( [ 'aioseo_description', 'aioseo_og_description', 'aioseo_twitter_description' ] as $h ) {
            add_filter( $h, $cb, 1 );
        }

        foreach ( [ 'seopress_titles_desc', 'seopress_social_og_desc', 'seopress_social_twitter_desc' ] as $h ) {
            add_filter( $h, $cb, 1 );
        }

        add_filter( 'jetpack_open_graph_tags', [ $this, 'strip_jetpack' ], 1 );
        add_action( 'wp_head', [ $this, 'fallback_og' ], 5 );
    }

    public function strip_wpcm( $t ) {
        if ( ! is_string( $t ) ) return $t;
        $t = preg_replace( '/\[wpcm_canvas[^\]]*\]/i', '', $t );
        $t = str_ireplace( '[/wpcm_canvas]', '', $t );
        return trim( preg_replace( '/\s+/', ' ', $t ) );
    }

    public function strip_jetpack( $tags ) {
        if ( ! is_array( $tags ) ) return $tags;
        foreach ( [ 'og:description', 'twitter:description' ] as $k ) {
            if ( isset( $tags[ $k ] ) ) $tags[ $k ] = $this->strip_wpcm( $tags[ $k ] );
        }
        return $tags;
    }

    public function fallback_og(): void {
        if ( ! is_singular() ) return;
        if ( defined( 'WPSEO_VERSION' ) || class_exists( '\\RankMath' ) || defined( 'AIOSEO_VERSION' ) || function_exists( 'seopress_init' ) ) return;

        global $post;
        if ( ! $post ) return;

        $txt  = $this->strip_wpcm( $post->post_content );
        $txt  = wp_strip_all_tags( strip_shortcodes( $txt ) );
        $desc = wp_trim_words( $txt, 30, '…' );
        if ( ! $desc ) return;

        $safe = esc_attr( $desc );
        echo '<meta property="og:description" content="' . $safe . '" />' . "
";
        echo '<meta name="twitter:description" content="' . $safe . '" />' . "
";
        echo '<meta name="description" content="' . $safe . '" />' . "
";
    }
}
